...
input_list.append([i[0] for i in StandardScaler().fit_transform(huobidm_input.loc[:, ['past_return10']].values)])
...
targets = np.array(huobidm_input['signal10'][:int(len(input_list[0])*0.5)], dtype='int64')
pca = PCA(n_components=2)
input_pca = pca.fit_transform(np.array(pd.DataFrame(inputs)))
input_pca_Df = pd.DataFrame(data = input_pca, columns = ['principal component 1', 'principal component 2'])
component1 = [i[0] for i in list(np.array(input_pca_Df))]
component2 = [i[1] for i in list(np.array(input_pca_Df))]



# plt.scatter(component1, component2, c=list(huobidm_input['signal10'])[:int(len(input_list[0])*0.5)], s=3)
# plt.show()

# exit()

# inputs = torch.from_numpy(inputs)
inputs = torch.from_numpy(input_pca)
...
#targets = np.array(list(huobidm_input['return10'])[:int(len(input_list[0])*0.5)], dtype='float32')
targets = np.array(list(huobidm_input['signal10'])[:int(len(input_list[0])*0.5)], dtype='float32')
